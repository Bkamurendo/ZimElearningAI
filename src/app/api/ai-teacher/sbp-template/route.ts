...
export async function POST(req: NextRequest) {
  try {
    ...
    // Check premium plan — pro, elite, and teachers with starter+ can access
    const { data: profile } = await supabase
      .from('profiles')
      .select('plan, role')
      .eq('id', user.id)
      .single() as { data: { plan: string | null; role: string | null } | null; error: unknown }

    const paidPlans = ['starter', 'pro', 'elite']
    if (!profile?.plan || !paidPlans.includes(profile.plan)) {
...
- [3-4 bullet points]`
    ...
    // Build a short WhatsApp-friendly summary (first ~500 chars of proposal stage)
    const proposalMatch = template.match(/## STAGE 1: PROPOSAL\s+([\s\S]*?)(?:### 🔍|---)/i)
    const proposalSnippet = proposalMatch
      ? proposalMatch[1].replace(/\[EXAMPLE[^\]]*\]/g, '').trim().substring(0, 400)
      : ''
    const whatsappText = `📚 *SBP Package — ${subject} (${grade})*\n\n*Topic:* ${topic}${resolvedHeritageTheme !== 'Zimbabwe heritage and local community' ? `\n*Heritage Theme:* ${resolvedHeritageTheme}` : ''}\n\n*Proposal Preview:*\n${proposalSnippet}...\n\n_Generated by ZimLearn AI — Zimbabwe's Heritage-Based Curriculum platform_\n🌐 zimlearn.app`

    return NextResponse.json({ template, subject, grade, topic, heritage_theme: resolvedHeritageTheme, whatsappText })
  } catch (err) {
    return NextResponse.json(
