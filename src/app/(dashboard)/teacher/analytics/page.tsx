...
import { Users, TrendingDown, AlertTriangle, CheckCircle2, Clock } from 'lucide-react'
    ...
    <div className="min-h-screen bg-gray-50/50">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6 sm:py-8 space-y-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Performance insights across your subjects</p>
        </div>
        ...
        {strugglingStudents.length > 0 && (
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <div className="flex items-center gap-2 px-5 py-4 border-b border-gray-50 bg-red-50/50">
              ...
              <h2 className="text-sm font-semibold text-red-800">Needs Attention ({strugglingStudents.length})</h2>
            </div>
            <div className="divide-y divide-gray-50">
              {strugglingStudents.slice(0, 8).map(s => (
                <div key={s.id} className="flex items-center justify-between px-5 py-3.5">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 bg-gradient-to-br from-red-400 to-rose-500 rounded-lg flex items-center justify-center flex-shrink-0">
                      <span className="text-white text-xs font-bold">{s.name[0]}</span>
                    </div>
                    <div>
                      <p className="text-sm font-semibold text-gray-900">{s.name}</p>
                      <p className="text-xs text-gray-400">{s.struggling} weak topics · {s.attemptCount} quizzes</p>
                    </div>
                  </div>
        ...
        {inactiveStudents.length > 0 && (
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <div className="flex items-center gap-2 px-5 py-4 border-b border-gray-50 bg-amber-50/50">
              ...
              <h2 className="text-sm font-semibold text-amber-800">Inactive 7+ Days ({inactiveStudents.length})</h2>
            </div>
            <div className="divide-y divide-gray-50">
        ...
        {topStudents.length > 0 && (
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <div className="flex items-center gap-2 px-5 py-4 border-b border-gray-50 bg-emerald-50/50">
              ...
              <h2 className="text-sm font-semibold text-emerald-800">Top Performers ({topStudents.length})</h2>
            </div>
            <div className="divide-y divide-gray-50">
              {topStudents.slice(0, 5).map((s, i) => (
                <div key={s.id} className="flex items-center justify-between px-5 py-3.5">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-gray-900">{s.name}</p>
                      <p className="text-xs text-gray-400">{s.mastered} topics mastered · {s.streak}d streak</p>
                    </div>
                  </div>
        ...
        {studentIds.length === 0 && (
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm text-center py-16">
            ...
            <p className="font-semibold text-gray-600">No student data yet</p>
