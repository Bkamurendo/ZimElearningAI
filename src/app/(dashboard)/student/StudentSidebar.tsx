...
const NAV = [
  ...
  { href: '/student/grade-predictor', label: 'Grade Predictor', icon: Target },
  { href: '/student/study-planner', label: 'Study Planner', icon: CalendarDays },
  { href: '/student/solver', label: 'Problem Solver', icon: Calculator },
]
...
export default function StudentSidebar({ userName, streak }: Props) {
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-5 border-b border-gray-100">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 bg-gradient-to-br from-green-500 to-green-700 rounded-xl flex items-center justify-center shadow-sm flex-shrink-0">
              <GraduationCap size={18} className="text-white" />
            </div>
            <div>
              <p className="font-bold text-gray-900 text-sm leading-tight">ZimLearn</p>
              <p className="text-xs text-green-600 font-medium">Student Portal</p>
            </div>
          </div>
          <button
            onClick={() => setOpen(false)}
