...
import { MessageSquare, Send, Inbox, ArrowLeft, X, Loader2, RefreshCw } from 'lucide-react'
...
export default function StudentMessagesPage() {
  ...
  // Load all teachers for compose dropdown
  useEffect(() => {
    supabase
      .from('profiles')
      .select('id, full_name')
      ...
      .order('full_name')
      .limit(100)
      .then(({ data }) => setTeachers((data ?? []) as TeacherRow[]))
  ...
  async function send() {
    if (!form.recipient_id) { setSendError('Please select a teacher'); return }
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <div className="max-w-2xl mx-auto px-4 sm:px-6 py-6 sm:py-8 space-y-5">
        ...
        {/* Header */}
        <div className="relative bg-gradient-to-br from-green-600 to-emerald-700 text-white rounded-2xl p-6 overflow-hidden">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-green-200 text-sm mt-0.5">
                  {inboxUnread > 0 ? `${inboxUnread} unread` : 'Chat with your teachers'}
                </p>
              </div>
            </div>
            <button
              onClick={() => { setComposing(true); setSendError('') }}
              className="flex items-center gap-2 bg-white text-green-700 hover:bg-green-50 font-semibold text-sm px-4 py-2.5 rounded-xl transition shadow-sm flex-shrink-0"
            >
              <Send size={15} />
              New Message
            </button>
          ...
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
              <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
                ...
                <button
                  onClick={() => { setComposing(false); setSendError(''); setForm({ recipient_id: '', content: '' }) }}
              ...
              <div className="p-5 space-y-4">
                ...
                <button
                  onClick={send}
                  disabled={sending}
                  className="w-full bg-green-600 hover:bg-green-700 disabled:opacity-50 text-white font-semibold py-2.5 rounded-xl transition flex items-center justify-center gap-2"
                >
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          ...
          <div className="flex items-center gap-0 px-4 pt-4 border-b border-gray-100">
            ...
            <button
              onClick={load}
              className="p-2 hover:bg-gray-100 rounded-lg transition mb-1 mr-1"
              title="Refresh"
            >
          ...
          {loading ? (
            ...
            <div className="text-center py-16 px-6">
              ...
              <p className="font-semibold text-gray-600">
                {box === 'inbox' ? 'Your inbox is empty' : 'Nothing sent yet'}
              </p>
              <p className="text-sm text-gray-400 mt-1">
                {box === 'inbox'
                  ? 'Messages from teachers will appear here'
                  : 'Tap "New Message" to message a teacher'}
              </p>
            </div>
          ) : (
              ...
              {messages.map(msg => {
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left flex gap-3 px-5 py-4 hover:bg-gray-50/80 transition ${isUnread ? 'bg-green-50/30' : ''}`}
                  >
