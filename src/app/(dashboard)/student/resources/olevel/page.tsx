...
const SUBJECT_EMOJI: Record<string, string> = {
  ...
  chemistry: '🧪', biology: '🧬', history: '🏛️', geography: '🌍',
  commerce: '💼', accounts: '📊', economics: '📈', literature: '📖',
  divinity: '✝️', agriculture: '🌱', 'computer science': '💻',
  art: '🎨', music: '🎵', shona: '🗣️', ndebele: '🗣️',
  french: '🇫🇷', spanish: '🇪🇸', 'business studies': '💼',
  'food and nutrition': '🍽️', 'fashion and fabrics': '👗',
}
...
// O-Level subject groupings
const SUBJECT_GROUPS: Record<string, string[]> = {
  'Core Subjects': ['english', 'mathematics'],
  'Sciences': ['physics', 'chemistry', 'biology', 'science', 'agriculture', 'computer science'],
  'Humanities': ['history', 'geography', 'divinity', 'literature', 'religious', 'shona', 'ndebele', 'french', 'spanish'],
  'Commercial': ['commerce', 'accounts', 'economics', 'business'],
  'Technical & Practical': ['food', 'fashion', 'art', 'music', 'physical'],
}
...
export default async function OLevelResourcesPage() {
  ...
  // Sort groups: put Core first
  const groupOrder = ['Core Subjects', 'Sciences', 'Humanities', 'Commercial', 'Technical & Practical', 'Other']
    ...
    <div className="min-h-screen bg-slate-50">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6 sm:py-8 space-y-6">
        ...
        <div className="flex items-center gap-2 text-sm flex-wrap">
          ...
          <ChevronRight size={14} className="text-gray-300" />
          <span className="font-semibold text-gray-900">O-Level</span>
        </div>

        {/* Hero */}
        <div className="relative bg-gradient-to-br from-blue-500 via-blue-600 to-indigo-700 text-white rounded-2xl p-6 overflow-hidden">
          ...
          <div className="relative">
            ...
            <h1 className="text-2xl font-bold">O-Level Resources</h1>
            <p className="text-blue-100 text-sm mt-1">
              Forms 1 – 4 · ZIMSEC Ordinary Level · {subjects.length} subject{subjects.length !== 1 ? 's' : ''} · {totalDocs} resource{totalDocs !== 1 ? 's' : ''}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-start gap-3 bg-blue-50 border border-blue-200 rounded-2xl p-4">
          ...
          <div>
            <p className="text-sm font-semibold text-blue-800">O-Level Examinations</p>
            <p className="text-xs text-blue-700 mt-0.5">
              ZIMSEC O-Level exams are taken at the end of Form 4. Students typically sit 8–10 subjects.
              Past papers from previous years are the best revision resource available.
            </p>
          </div>
        </div>
        ...
        {sortedGroups.length === 0 ? (
          <div className="bg-white rounded-2xl border border-dashed border-gray-200 p-12 text-center shadow-sm">
            ...
            <p className="font-semibold text-gray-600">No O-Level subjects found</p>
            <p className="text-sm text-gray-400 mt-1">O-Level subjects will appear here once added.</p>
          </div>
        ) : (
                    ...
                    {groupSubjects.map((subject) => {
                      ...
                      return (
                        <Link
                          ...
                          className="group bg-white rounded-2xl border border-gray-100 overflow-hidden hover:border-blue-300 hover:shadow-lg transition-all duration-200 shadow-sm"
                        >
                          <div className="flex items-center gap-3 p-4 border-b border-gray-50">
                            <div className="w-11 h-11 bg-blue-50 border border-blue-200 rounded-xl flex items-center justify-center text-xl flex-shrink-0">
                              {emoji}
                            </div>
                            <div className="flex-1 min-w-0">
                              <h3 className="font-semibold text-gray-900 text-sm leading-tight">{subject.name}</h3>
                              <p className="text-xs text-gray-400 mt-0.5">O-Level · {total} resource{total !== 1 ? 's' : ''}</p>
                            </div>
