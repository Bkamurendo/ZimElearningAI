  ...
  return (
    <div className="min-h-screen bg-slate-50/50">
      ...
      <div className="bg-white border-b border-slate-100 px-6 py-4 sticky top-12 z-40 backdrop-blur-md bg-white/80">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          ...
          <div className="hidden sm:flex items-center gap-2">
            {[1,2,3].map(i => (
              <div key={i} className="w-8 h-8 rounded-full border-2 border-white bg-slate-200 flex items-center justify-center text-xs font-black text-slate-500 shadow-sm">U</div>
            ))}
      ...
      <div className="max-w-6xl mx-auto p-4 sm:p-6 grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        {/* Sidebar Nav — horizontal tabs on mobile, vertical on desktop */}
        <div className="lg:col-span-1">
          <div className="flex lg:flex-col gap-2 overflow-x-auto pb-1 lg:pb-0 -mx-4 px-4 lg:mx-0 lg:px-0">
          {[
            { id: 'chat', label: 'Chat', icon: MessageSquare, color: 'text-indigo-600' },
            { id: 'library', label: 'Library', icon: Library, color: 'text-emerald-600' },
            { id: 'mastery', label: 'Mastery', icon: Target, color: 'text-rose-600' }
          ].map(tab => (
            <button
              key={tab.id}
              onClick={() => setActiveTab(tab.id as any)}
              className={`flex-shrink-0 lg:w-full flex items-center gap-2 lg:gap-3 px-4 py-3 rounded-2xl transition-all font-bold text-sm min-h-[44px] ${
          ...
          <div className="hidden lg:block mt-6 pt-6 border-t border-slate-100">
             <div className="bg-gradient-to-br from-indigo-600 to-purple-700 rounded-2xl p-4 text-white shadow-xl shadow-indigo-100 relative overflow-hidden">
                ...
                <p className="text-xs text-indigo-100 font-medium mb-3">Reach 80% Mastery in "Calculus - Derivatives" together.</p>
        ...
        {/* Main Content Area */}
        <div className="lg:col-span-3 h-[60vh] lg:h-[70vh] flex flex-col">
                 ...
                 <div className="flex-1 overflow-y-auto p-4 space-y-4">
                   {messages.map((msg) => (
                     <div key={msg.id} className="flex items-start gap-3">
                       <div className="w-8 h-8 rounded-lg bg-slate-100 flex-shrink-0 flex items-center justify-center text-xs font-bold uppercase">
             ...
             {activeTab === 'library' && (
               ...
               >
                 <div className="flex items-center justify-between px-2">
                   ...
                   <button className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-black uppercase tracking-widest px-4 py-2 rounded-xl transition shadow-lg shadow-emerald-100">
                 ...
                 <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                   {resources.length === 0 ? (
                     <div className="col-span-full py-16 text-center bg-white rounded-3xl border border-dashed border-slate-200">
                        ...
                        <p className="text-slate-300 text-xs mt-1 uppercase tracking-widest font-black">Share a past paper or notes to begin.</p>
                     </div>
                   ) : (
                     resources.map(res => (
                ...
                >
                  ...
                  <div className="w-full max-w-sm space-y-4 pt-4">
                     <div className="flex items-center justify-between">
                       <span className="text-xs font-black text-slate-500 uppercase tracking-widest">Aggregate Strength</span>
