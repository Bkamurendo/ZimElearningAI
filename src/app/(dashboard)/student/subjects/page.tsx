...
export default async function StudentSubjectsPage() {
  ...
  const { data: sp } = await supabase
    .from('student_profiles').select('id, zimsec_level, grade').eq('user_id', user.id).single() as {
    data: { id: string; zimsec_level: string; grade: string } | null; error: unknown
  }

  const { data: enrolments } = await supabase
    .from('student_subjects').select('subject_id').eq('student_id', sp?.id ?? '') as {
  ...
  if (subjectIds.length > 0) {
    const { data: subjectData } = await supabase
      .from('subjects').select('id, name, code, zimsec_level')
      .in('id', subjectIds) as { data: { id: string; name: string; code: string; zimsec_level: string }[] | null; error: unknown }

    for (const s of subjectData ?? []) {
      const { count: courseCount } = await supabase
        .from('courses').select('id', { count: 'exact', head: true }).eq('subject_id', s.id).eq('published', true)
      const { count: resourceCount } = await supabase
        .from('uploaded_documents').select('id', { count: 'exact', head: true })
        .eq('subject_id', s.id).eq('moderation_status', 'published')

      subjects.push({ ...s, course_count: courseCount ?? 0, resource_count: resourceCount ?? 0 })
    }
  }
