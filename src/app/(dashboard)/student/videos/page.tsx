...
export default async function VideosPage() {
  ...
  const { data: studentProfile } = await supabase
    .from('student_profiles').select('id, zimsec_level').eq('user_id', user.id).single() as { data: { id: string; zimsec_level: string } | null; error: unknown }
  ...
  type VideoRow = { id: string; title: string; description: string | null; youtube_id: string | null; topic: string | null; duration_mins: number | null; subject: { name: string; code: string } | null }
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {vids.map(video => (
                  ...
                  >
                    ...
                    <div className="p-4">
                      <p className="font-semibold text-gray-900 text-sm line-clamp-2 group-hover:text-blue-600 transition-colors">{video.title}</p>
                      {video.topic && <p className="text-xs text-gray-400 mt-1">{video.topic}</p>}
                    </div>
                  </Link>
                ))}
              </div>
