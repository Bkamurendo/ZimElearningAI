  ...
  try {
    ...
    if (!myEntry && myProfile) {
      const { data: myStreak } = await supabase
        ...
        .single() as { data: { total_xp: number; current_streak: number } | null; error: unknown }
        
      const { count } = await supabase
        .from('student_streaks')
        .select('id', { count: 'exact', head: true })
        .gt('total_xp', myStreak?.total_xp ?? 0)
    ...
    console.error('[Leaderboard] Runtime error:', err)
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-4 px-4 text-center bg-gray-50">
        ...
        <h2 className="text-2xl font-bold text-slate-800">Couldn&apos;t load leaderboard</h2>
        <p className="text-slate-500 max-w-xs">We encountered an error while fetching the rankings. Please refresh the page.</p>
        <Link href="/login">
